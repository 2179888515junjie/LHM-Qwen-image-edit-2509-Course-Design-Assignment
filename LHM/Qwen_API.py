...
def demo_lhm(pose_estimator, face_detector, parsing_net, lhm, motion_generation, cfg):
    ...
    def core_fn(image: np.ndarray, video_params, working_dir):
        ...
        from qwen_edit import qwen_image_edit
    ...
    with gr.Blocks(analytics_enabled=False) as demo:
        ...
        # 统一的样式：让 Image 的 canvas & video 自适应
        gr.HTML("""
        <style>
        /* 让三个 Image 的 canvas 宽度自适应，比例保持不变 */
        #input-img canvas,
        ...
        with gr.Row():
            with gr.Column(scale=1):
                input_image = gr.Image(
                    ...
                    type="numpy",
                    sources="upload",
                    elem_id="input-img",  # ✅ 关键：给 CSS 用的 id
                    height=None,  # ✅ 不固定高度
                    container=True  # ✅ 使用容器布局
                )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                video_input = gr.Video(
                    label="Target Motion",
                    sources="upload",
                    elem_classes="auto-resize",  # ✅ 用上面 .auto-resize video 的样式
                    value=default_video,
                )
